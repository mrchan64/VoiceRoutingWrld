...
var locationExc = "?!here|my.* location|me|where I am"
regexPatts = [ /^.*[Ww]here\s(?:is\s)*(.*?)(?:\sis)*\s*$/,
			/^.*[Ff]rom\s(${locationExc})(.*)\sto\s(.*)\s*$/, //accounting for 'my current location'
			/^.*[Tt]o\s(.*)\sfrom\s(${locationExc})(.*)\s*$/,
			/^.*to\s(.*(?!from ${locationExc}))\s*$/,	
