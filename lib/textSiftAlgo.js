...
/*
 COMMAND TYPES{
	0: Location lookup
	1: Double-ended route
	2: Route from current location
	3: Nearby location lookup
 } 
...
regexPatts = [ /^.*[Ww]here\s(?:is\s)*(.*?)(?:\sis)*\s*$/,		// Location lookup 
			/^.*[Ff]rom\s(${locationExc})(.*)\sto\s(.*)\s*$/, 	// Double-ended route
			/^.*[Tt]o\s(.*)\sfrom\s(${locationExc})(.*)\s*$/,	// Double-ended route
			/^.*to\s(.*(?!\sfrom ${locationExc}))\s*$/,			// Route from curr location  NEEDS FIXING
			/^.*(?:some|all)\s*(.*)\s(?:nearby|near\sme|near\sus)\s*$/,	// Nearby location lookup
			/^.*(?:nearby|near)\s(.*)\s*$/,						// Nearby location lookup
...
exports.findCommand = function(inputString){
	...
	switch (index) {
		...
		case 3:
			...
			break;
		case 4:
			// NEARBY: shows all nearby matching locations
			returnArr.push(3);
			returnArr.push(match[1]); // LOCATION name e.g. Starbucks
...
inputString = "Show me all El Dorados nearby"
cmd = exports.findCommand(inputString);
console.log("\"" + inputString + "\"")
console.log(cmd);
